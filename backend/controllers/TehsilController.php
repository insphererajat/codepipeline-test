 ...
 */
class TehsilController extends AdminController
{
    ...
    {
        $searchModel = new MstTehsilSearch;
    ...
    public function actionCreate()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->load(Yii::$app->request->post()) && $model->validate() && $model->save()) {
                ...
                return $this->redirect(\yii\helpers\Url::toRoute(['tehsil/index']));
            }
        }
    ...
    public function actionDelete($guid)
    {
        ...
        try {
            ...
            $this->setSuccessMessage('Tehsil has been deleted successfully.');
            return $this->redirect(\yii\helpers\Url::toRoute(['district/index']));
        }
        catch (\Exception $ex) {
            throw new \components\exceptions\AppException($ex->getMessage());
    ...
    {
        $model = MstTehsil::findByGuid($guid, ['resultFormat' => \common\models\caching\ModelCache::RETURN_TYPE_OBJECT]);
        if ($model == null) {
            throw new \components\exceptions\AppException("Oops! You trying to access tehsil doesn't exist or deleted.");
        }
        return $model;
    }
