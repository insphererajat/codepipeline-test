...
class CscWallet extends Component
{
    ...
    public $furlFee;
    public $productId;
    public $authorizationEndpointUrl = "https://connect.csc.gov.in/account/authorize"; //"https://connect.csc.gov.in/account/authorize";
    public $tokenEndpointUrl = "https://connect.csc.gov.in/account/token";
    public $resourceUrl = "https://connect.csc.gov.in/account/resource";
    public $walletUrl = "https://connect.csc.gov.in/account/resource";

    public function __construct($config = [])
    {
        $mstConfigModel = MstConfiguration::findByType(MstConfiguration::TYPE_PAYMENT_CSC, ['isActive' => ModelCache::IS_ACTIVE_YES]);
        if(empty($mstConfigModel)) {
            throw new AppException("Invalid request.");
        }
        $this->cscObj = new CscBridge([
            'publicKey' => Helper::decryptString($mstConfigModel['config_val9']),
            'privateKey' => Helper::decryptString($mstConfigModel['config_val10'])
        ]);
        
        $this->paymentMethod = $config['paymentMethod'];
        $this->clientId = Helper::decryptString($mstConfigModel['config_val1']);
        $this->clientSecret = Helper::decryptString($mstConfigModel['config_val2']);
        $this->clientToken = Helper::decryptString($mstConfigModel['config_val3']);
        $this->productId = Helper::decryptString($mstConfigModel['config_val4']);
        $this->cscConnectRedirectUrl = Helper::decryptString($mstConfigModel['config_val5']);
        $this->surlFee = Helper::decryptString($mstConfigModel['config_val6']);
        $this->furlFee = Helper::decryptString($mstConfigModel['config_val7']);
        $this->merchantKey = Helper::decryptString($mstConfigModel['config_val8']);
        $this->publicKey = Helper::decryptString($mstConfigModel['config_val9']);
        $this->privateKey = Helper::decryptString($mstConfigModel['config_val10']);
        $this->authorizationEndpointUrl = Helper::decryptString($mstConfigModel['config_val11']);
        $this->tokenEndpointUrl = Helper::decryptString($mstConfigModel['config_val12']);
        $this->resourceUrl = Helper::decryptString($mstConfigModel['config_val13']);
        $this->walletUrl = Helper::decryptString($mstConfigModel['config_val14']);
    ...
    public function initParams()
    {
        return [
            ...
            'product_name' => 'Yogyata Application',
            'txn_amount' => '1150',
            'amount_parameter' => 'NA',
            'txn_mode' => 'D',
            'txn_type' => 'D',
            ...
            'csc_share_amount' => '0',
            'pay_to_email' => 'varun@yogya-ta.com',
            'return_url' => '',
            'cancel_url' => '',
            'Currency' => 'INR',
            'Discount' => '0',
    ...
    public function get_enquiry($tid)
    {
        if (!isset($this->merchant_id)) {
            throw new AppException("Merchant ID not set. Please call set_mid first.");
        }
        $data = [
            'merchant_txn' => $tid
        ];
...
//			CURLOPT_CUSTOMREQUEST => 'PUT',
    ...
    public function fetch_data($url, $post, $heads)
    {
        ...
        $result = curl_exec($curl);
        if (curl_errno($curl)) {
            throw new AppException(curl_error($curl));
        }
        curl_close($curl);
