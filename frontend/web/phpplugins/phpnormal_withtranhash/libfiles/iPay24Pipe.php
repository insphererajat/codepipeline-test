...
class iPay24Pipe {
	...
	protected $trandate = "";
	protected $tranhashkey = "";
	protected $tranhash = "";
	protected $trandata = "";
	protected $tranhashrec = "";
	...
	function gettranhashkey() {
		return $this->tranhashkey;
	}
	function gettranhash() {
		return $this->tranhash;
	}
	function gettrandata() {
		return $this->trandata;
	}
	function gettranhashrec() {
		return $this->tranhashrec;
	}
	...
	function settranhashkey($val) {
		$this->tranhashkey = $val;
	}
	function settranhash($val) {
		$this->tranhash = $val;
	}
	function settrandata($val) {
		$this->trandata = $val;
	}
	function settranhashrec($val) {
		$this->tranhashrec = $val;
	}
	// ##############################################################################################################################
	...
	function buildHostRequest() {
		...
		try {
			...
			$this->tranhashkey = substr(str_shuffle("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_@"), 0, 16);
			$strRequest .= "tranhashkey=" . $this->tranhashkey . "&";
	...
	function buildXMLRequest() {
		...
		try {
			...
			$this->tranhashkey = substr(str_shuffle("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_@"), 0, 16);
			$requestbuffer .= "<tranhashkey>" . $this->tranhashkey . "</tranhashkey>";
	...
	function performPaymentInitializationHTTP() {
		...
		try {
			...
			$this->tranhash = base64_encode(hash_hmac('sha256', $requestbuffer, $this->tranhashkey));
			$request .= "&trandata=" . $cipheredText;
			$request .= "&errorURL=" . $this->errorURL . "&responseURL=" . $this->responseURL . "&tranportalId=" . $xmlData ['id'] . "&tranhash=" . $this->tranhash;
	...
	function performTransaction() {
		...
		try {
			...
			$encrrequestbuffer.="<trandata>" . $this->encryptData ( $requestbuffer, $this->key ) . "</trandata><tranhash>" . base64_encode(hash_hmac('sha256', $requestbuffer, $this->tranhashkey)) . "</tranhash><id>" . $xmlData ['id'] . "</id><password>" . $xmlData ['password'] . "</password>";
			...
			$response = $pipe->performTranPortalTransaction ( $encrrequestbuffer, $webaddr );
			...
			$this->setTransactionData ( $resultMap );
			
			$this->tranhashrec = $this->tranhash;
			
			$response = $this->decryptData ( $this->trandata, $this->key );
			 
			$resultMap = $this->parseResponse ( $response );
			 
			$this->setTransactionData ( $resultMap );
			
			$this->tranhash = base64_encode(hash_hmac('sha256', $response, $this->tranhashkey));
	...
	function performVbVTransaction() {
		...
		try {
			...
			$xmlData = $this->parseResource ( $this->key, $this->resourcePath, $this->alias );
			if ($xmlData == null) {
				$this->error = "Alias name does not exist";
				return - 1;
			} else {
				$hm = $this->parseXMLRequest ( $xmlData );
			}
			...
			$request = "&trandata=" . $cipheredText . "&errorURL=" . $this->errorURL . "&responseURL=" . $this->responseURL . "&tranportalId=" . $hm ['id'] . "&tranhash=" . $this->tranhash;
	...
	function parseEncryptedRequest($trandata) {
		...
		try {
			...
			$this->tranhash = base64_encode(hash_hmac('sha256', $trandata, $this->tranhashkey));
	...
	function setTransactionData($resultMap) {
		...
		if (isset ( $resultMap ['tranhashkey'] ))
			$this->tranhashkey = trim ( $resultMap ['tranhashkey'] );
		if (isset ( $resultMap ['tranhash'] ))
			$this->tranhash = trim ( $resultMap ['tranhash'] );
		if (isset ( $resultMap ['trandata'] ))
			$this->trandata = trim ( $resultMap ['trandata'] );
	...
	function parsetrandata($trandata) {
		try {
			...
			if (isset ( $splitData ['trandate'] )) {
				$this->trandate = $splitData ['trandate'];
			}
			if (isset ( $splitData ['tranhashkey'] )) {
				$this->tranhashkey = $splitData ['tranhashkey'];
			}
		} catch ( Exception $e ) {
