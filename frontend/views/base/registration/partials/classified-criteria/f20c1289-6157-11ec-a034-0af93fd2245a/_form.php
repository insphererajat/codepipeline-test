...
<div class="col-12 p-0">
            ...
            <div class="c-form c-form-xs col2 u-fieldRadius8 u-fieldShadow u-fieldBorderClr2 mt-3 section-<?= $postId ?> <?= isset($model->posts[$postId]['post_id']) ? '' : 'hide' ?>">
                ...
                foreach ($optionsSeq as $key => $seq):
                    ...
                    foreach ($seq as $key => $pq) {
                        $field1 = '';
                        if (!empty($pq['qualification_id'])) {
                            $options[$pq['qualification_id']] = MstQualification::getName($pq['qualification_id']);
                        } else if (!empty($pq['field1'])) {
                            $options[$pq['field1']] = $pq['field1'];
                        }
                        if (ArrayHelper::isIn($postId, [8]) && $c == 2) {
                            $class = "";
                        }
                    }
                    ?>
    ...
    if (!$flag):
        ...
        <div class="c-sectionHeader c-sectionHeader-xs design3">
            <div class="c-sectionHeader__container">
                <div class="c-sectionHeader__label">
                    <div class="c-sectionHeader__label__title fs16__medium"><span class="alert alert-info">Sorry! you are not eligible for this advertisement due to age, etc. Please contact with support team.</span></div>
                </div>
            </div>
        </div>
<?php endif; ?>
